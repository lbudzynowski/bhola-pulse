...
class MonitorDiscoveryTests(unittest.TestCase):
    def test_parses_indices_without_retaining_monitor_names(self) -> None:
        output = """Monitors: 3
...
"""
    ...
    def test_single_monitor_and_empty_output_fall_back_safely(self) -> None:
        self.assertEqual(
            parse_active_monitors("Monitors: 1\n 0: +*only 1920/1x1080/1+0+0  only\n"),
            [0],
        )
        self.assertEqual(parse_active_monitors(""), [0])

    def test_command_failure_and_timeout_fall_back_to_one_instance(self) -> None:
    ...
    def test_discovery_uses_bounded_read_only_command(self) -> None:
        ...
        def runner(command, **kwargs):
            ...
            return subprocess.CompletedProcess(command, 0, "Monitors: 2\n 0: +a\n 1: +b\n", "")

        self.assertEqual(discover_active_monitors(runner), [0, 1])
